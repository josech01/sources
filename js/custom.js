...
　 }
    ...
    $rentForm.submit(function(){
        ...
        var $success      = '<strong>Su mensaje fue enviado correctamente.';
	    var $error      = '<strong>Error!</strong> Your message was not sent - try again later...';
      	var response;

        $.ajax({
            type: "POST",
            url: "/php/rent-form.php",
            data: $(this).serialize(),
            success: function(msg) {
                if (msg === 'SEND') {
                    response = '<div class="alert alert-success">'+ $success +'</div>';
    ...
    function recalculate(){
        ...
        // $('#sumatotal').html(sum);
        // $('input.sumatotal').val(sum);
        $('#sumatotal').attr('value', sum);
        $('#sumatotal').attr('placeholder', sum);
        
        $('select').change(function(){
            var multi = 0;
            $('select :selected').each(function() {
                multi += Number($(this).val());
            });
            $("#multi").html("El valor total de su propuesta es:"+ "<strong>"+multi*sum+"</strong>");
        });
    }
